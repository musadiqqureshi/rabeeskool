...
const sections: { label: string; items: { href: string; label: string; icon: string }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/products", label: "Products", icon: "M3 7l9-4 9 4-9 4-9-4zM3 7v10l9 4 9-4V7M12 11v10" },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/goals", label: "Goals", icon: "M12 12m-9 0a9 9 0 1 0 18 0 9 9 0 1 0-18 0M12 12m-4.5 0a4.5 4.5 0 1 0 9 0 4.5 4.5 0 1 0-9 0M12 12h.01" },
...
export function Sidebar({ academyName, fullName, slug }: { academyName: string; fullName: string | null; slug: string }) {
  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-line bg-white">
      <div className="flex items-center justify-between px-5 py-4">
        ...
        <button type="button" aria-label="Notifications" className="relative rounded-lg p-1.5 text-muted hover:bg-slate-100 hover:text-ink">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 0 1-3.4 0" /></svg>
        </button>
      </div>

      <div className="px-3 pb-2">
        <div className="flex items-center gap-2 rounded-lg border border-line bg-slate-50 px-3 py-2 text-sm text-muted">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="m21 21-4.3-4.3" /></svg>
          <span className="flex-1">Search…</span>
          <kbd className="rounded border border-line bg-white px-1.5 text-[10px] font-medium text-slate-400">⌘K</kbd>
        </div>
      </div>
      ...
      <div className="px-3 pb-2">
        <Link
          href={slug ? `/a/${slug}` : "#"}
          className="flex items-center justify-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2.5 text-sm font-semibold text-emerald-700 transition-colors hover:bg-emerald-100"
        >
          View as student
